...
async function main() {
  ...
  try {
    if (shouldStartServer) {
      serverProcess = spawn(process.execPath, ['scripts/start-standalone.mjs'], {
        cwd: process.cwd(),
        stdio: 'inherit',
        env: {
          ...process.env,
    ...
    for (const smoke of CRITICAL_SMOKES) {
      ...
      try {
        runNodeScript([smoke.script, '--base-url', baseUrl, '--output-dir', smoke.outputDir]);
        const report = await readJson(path.join(process.cwd(), smoke.reportPath));
        results.push({
          id: smoke.id,
  ...
  const summaryPath = path.join(summaryDir, 'report.json');
