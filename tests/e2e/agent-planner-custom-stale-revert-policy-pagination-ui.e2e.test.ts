...
describe.sequential('Agent planner stale revert policy pagination UI e2e', () => {
  ...
  it('paginates allowlist audit events across pages', async () => {
    if (!server || !browser) {
    ...
    await expect
      .poll(
        () =>
          policyPanel
            .locator('[data-testid="agent-planner-stale-revert-policy-audit-entry"]')
            .textContent(),
        { timeout: 5_000 }
      )
      .toContain('Permitir EDITOR');
  }, 100_000);
});
